...
StreetW -> 'аллея'| 'бульвар'| 'вал'| 'взвоз'| 'въезд'| 'дорога'| 'заезд'| 'кольцо'| 'линия (линнея)'| 'луч'| 'магистраль'| 'набережная'| 'переулок'| 'перспектива(першпектива)'| 'площадь'| 'проезд'| 'проспект'| 'проулок'| 'разъезд'| 'спуск'| 'съезд'| 'территория'| 'тракт'| 'тупик'| 'улица'| 'шоссе';
StreetSokr -> 'ал' | 'бул' | 'б-р' | 'дор' | 'маг' | 'наб' | 'пер' | 'пл' | 'пр' | 'пр-д' | 'просп' | 'пр-кт' | 'туп' | 'ул' | 'ш';
